...
<div ng-show="result.processed && (result.success.length > 0 || result.failure.length > 0)"
    ...
    <div ng-show="result.success.length > 0">
        <h2 acs-coral-heading>{{ result.success.length }} property rows successfully processed</h2>
<%--
    ...
    <div ng-show="result.warning.length > 0">
        <h2 acs-coral-heading>{{ result.warning.length }} property rows resulted in warnings while being processed</h2>
        <%--
                <ul ng-repeat="path in result.success track by $index">
                    <li>{{ path }}</li>
                </ul>--%>
    ...
    <div ng-show="result.failure.length > 0">
        <h2 acs-coral-heading>{{ result.failure.length }} property rows failed to process</h2>
<%--
...
<div ng-show="result.processed && result.success.length === 0 && result.failure.length === 0" class="results">
    <h2 acs-coral-heading>The properties in the CSV file were unable to be applied to the specified Node.</h2>
